"""Executor do relatorio BRIEF-LOCAL-BEAU.

Le um CSV/XLSX local, seleciona a UF informada e monta um contrato de
renderizacao para o briefing visual.
"""
...
class BRIEFLOCALBEAUExecutor:
    """Executor para a versao visual do briefing local por UF."""

    def __init__(self, reports_dir: Path):
        self.report_dir = reports_dir / "BRIEF-LOCAL-BEAU"
    ...
    async def execute(self, data: dict[str, Any]) -> bytes:
        ...
        beau_data = self._build_beau_data(row)
        ...
        logger.info("Compilando briefing visual para UF %s", uf)
        return await self._compile_typst(template_data)

    def _build_beau_data(self, row: dict[str, Any]) -> dict[str, Any]:
        def pair(column: str) -> dict[str, str]:
            raw = self._clean_value(row.get(column))
            if raw is None:
                return {"local": "—", "brasil": "—"}
            local, brasil = self._split_value_brasil(raw)
            if brasil and brasil.lower().startswith("brasil:"):
                brasil = brasil.split(":", 1)[1].strip()
            return {
                "local": local or "—",
                "brasil": brasil or "—",
            }

        def connected_schools() -> dict[str, str]:
            values = pair("escolas_conectadas_nivel_4_5")
            ...
            local_count, local_pct = split(values["local"])
            brasil_count, brasil_pct = split(values["brasil"])
            return {
        ...
        def series(column: str) -> dict[str, Any]:
            raw = self._clean_value(row.get(column))
            if raw is None:
                return {"local": ["—"], "brasil": ["—"], "note": ""}
            local, brasil = self._split_value_brasil(raw)
            local, local_note = self._extract_note(local, {"note_prefixes": ["(*) 2026"]})
            brasil, brasil_note = self._extract_note(brasil, {"note_prefixes": ["(*) 2026"]})
            return {
                "local": self._split_series(local, {}) or ["—"],
                "brasil": self._split_series(brasil, {}, strip_brasil=True) or ["—"],
                "note": brasil_note or local_note or "",
            }

        return {
            "escolas": connected_schools(),
            "eti_matriculas": pair("eti_matriculas_fomentadas"),
            "eti_valor": pair("eti_valor_fomentado"),
            "eti_serie": series("eti_serie_historica_alunos"),
            "cnca_cantinhos": pair("cnca_cantinho_leitura"),
            "cnca_escolas": pair("cnca_escolas_apoiadas"),
            "cnca_valor_cantinhos": pair("cnca_valor_cantinhos"),
            "cnca_articuladores": pair("cnca_qtd_articuladores_total"),
            "cnca_valor_articuladores": pair("cnca_articuladores_renalpha_2026"),
            "cnca_materiais": pair("cnca_valor_materiais"),
            "cnca_formacao": pair("cnca_valor_formacao"),
            "cnca_total": pair("cnca_total_investido"),
            "ica": pair("ica_alfabetizacao"),
        }
    ...
    async def _compile_typst(self, template_data: dict[str, Any]) -> bytes:
        ...
        output_path = output_dir / "brief-local-beau.pdf"
