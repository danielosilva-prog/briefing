-- Percentual por condicionalidade (1 a 5) na UF do municipio informado.
...
base_uf AS (
  ...
  JOIN `br-mec-segape.educacao_dados_mestres.municipio` m
    ON CAST(m.id_municipio AS STRING) = CAST(fc.id_municipio AS STRING)
...
pct_calc AS (
  SELECT
    condicionalidade AS condicionalidadeUf,
    atendimento AS atendimentoUf,
    quantidade AS quantidadeUf,
    CASE
...
FROM pct_calc
ORDER BY
  CAST(condicionalidadeUf AS INT64),
  CASE atendimentoUf
    WHEN "true" THEN 1
    WHEN "false" THEN 2
    ELSE 3
